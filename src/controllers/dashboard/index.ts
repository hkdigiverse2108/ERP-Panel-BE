...
export const transactionDetails = async (req, res) => {
  try {
    ...
    const results: any = await Promise.all([
      ...
      PosOrderModel.aggregate([
        { $match: { ...dateCriteria, status: POS_ORDER_STATUS.COMPLETED } },
        {
          $group: {
            ...
            uniqueCustomers: {
              $addToSet: {
                $cond: [{ $ne: ["$customerId", null] }, "$customerId", "$$REMOVE"],
              },
            },
                ...
                $reduce: {
                  ...
                  in: {
                    $add: [
                      "$$value",
                      {
                        $multiply: ["$$this.qty", { $ifNull: ["$$this.unitCost", 0] }],
                      },
                    ],
                  },
                },
      ...
      supplierBillModel.aggregate([
        { $match: purchaseDateCriteria },
        {
          $group: {
            ...
            uniqueSuppliers: {
              $addToSet: {
                $cond: [{ $ne: ["$supplierId", null] }, "$supplierId", "$$REMOVE"],
              },
            },
      ...
      (async () => {
        const [vEx, pEx]: any = await Promise.all([
          voucherModel.aggregate([{ $match: { ...voucherDateCriteria, type: VOUCHAR_TYPE.EXPENSE } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
      ...
      (async () => {
        const [vPay, pPay]: any = await Promise.all([
          voucherModel.aggregate([{ $match: { ...voucherDateCriteria, type: VOUCHAR_TYPE.PAYMENT } }, { $group: { _id: null, total: { $sum: "$amount" } } }]),
          PosPaymentModel.aggregate([
            {
              $match: {
                ...dateCriteria,
    ...
    const sr: any = results[2][0] || {};
    const ex: any = results[3] || {};
    const totalProducts = results[4][0] || 0;
    const stockInfo = results[4][1] || [];
    const si: any = stockInfo[0] || {};
    const cih: any = results[5][0] || {};
    const bab: any = results[6][0] || {};
    const totalPaidData: any = results[7] || {};
    ...
    const result = {
      ...
      // Expense & Cash Flow
      totalPaid: Number((totalPaidData.totalPaid || 0).toFixed(2)),
      totalExpense: Number((ex.totalExpense || 0).toFixed(2)),
      cashInHand: Number((cih.totalCash || 0).toFixed(2)),
      bankAccountsBalance: Number((bab.totalBank || 0).toFixed(2)),
        ...
        $project: {
          ...
          customerId: {
            ...
            name: {
              $cond: {
                if: {
                  $or: [{ $ifNull: ["$customer.firstName", false] }, { $ifNull: ["$customer.lastName", false] }],
                },
                then: {
                  $trim: {
                    input: {
...
export const categoryWiseCustomers = async (req, res) => {
  try {
    ...
    let { startDate, endDate, companyFilter, typeFilter = "all", customerTypeFilter, customerFilter, branchFilter } = value;
          ...
          salesPercentage: {
            $cond: {
              ...
              then: 0,
                ...
                $round: [
                  {
                    $multiply: [{ $divide: ["$totalSalesValue", "$grandTotalSales"] }, 100],
                  },
                  2,
                ],
              },
...
export const receivable = async (req, res) => {
  try {
    ...
    if (branchId) {
      const branchObjId = new mongoose.Types.ObjectId(branchId as string);
    ...
    const [posOrders, invoiceData] = await Promise.all([
      PosOrderModel.aggregate([
        ...
        { $unwind: { path: "$customer", preserveNullAndEmptyArrays: true } },
        {
          $project: {
            customerName: {
              $concat: [{ $ifNull: ["$customer.firstName", "Walk-In"] }, " ", { $ifNull: ["$customer.lastName", ""] }],
            },
            invoiceNo: "$orderNo",
  ...
  try {
    ...
    const data = await supplierBillModel.aggregate([
      ...
      { $unwind: { path: "$supplier", preserveNullAndEmptyArrays: true } },
      {
        $project: {
          supplierName: {
            $concat: [{ $ifNull: ["$supplier.firstName", "Unknown Supplier"] }, " ", { $ifNull: ["$supplier.lastName", ""] }],
          },
...
export const getRenewalDueOfMonth = async (req, res) => {
  try {
    ...
    if (search) {
      criteria.$or = [{ name: { $regex: search, $options: "si" } }, { displayName: { $regex: search, $options: "si" } }];
    }
    ...
    applyDateFilter(criteria, startDate as string, endDate as string, "planEndDate");
