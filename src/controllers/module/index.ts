...
export const get_all_module = async (req, res) => {
  ...
  try {
    ...
    if (search) {
      criteria.$or = [{ tabName: { $regex: search, $options: "si" } }, { displayName: { $regex: search, $options: "si" } }];
    }
    ...
    return res.status(HTTP_STATUS.OK).json(
      new apiResponse(
        HTTP_STATUS.OK,
        responseMessage?.getDataSuccess("module"),
        {
...
export const get_users_permissions_by_moduleId = async (req, res) => {
  ...
  try {
    ...
    if (search) {
      match.$or = [{ fullName: { $regex: search, $options: "si" } }, { email: { $regex: search, $options: "si" } }];
    }

    const users = await getData(userModel, match, { password: 0 }, {});
    const userIds = users.map((u: any) => u._id);

    const perms = await getData(permissionModel, { moduleId: new ObjectId(moduleId), userId: { $in: userIds }, isDeleted: false }, {}, {});
