...
export const updateReportFormat = async (req: Request | any, res: Response | any) => {
  ...
  try {
    ...
    if (updateFields.type !== undefined) updateDataPayload["reportFormats.$.type"] = updateFields.type;
    if (updateFields.formats !== undefined) updateDataPayload["reportFormats.$.formats"] = updateFields.formats;
    if (updateFields.isActive !== undefined) updateDataPayload["reportFormats.$.isActive"] = updateFields.isActive;
...
export const getAllReportFormats = async (req: Request | any, res: Response | any) => {
  reqInfo(req);
  try {
    const { search, activeFilter } = req.query;
    let query: any = { isDeleted: false };

    const existingDoc = await getFirstMatch(settingsModel, query, { reportFormats: 1 }, {});
    if (!existingDoc) {
    ...
    if (activeFilter !== undefined) {
      const isActiveFilter = activeFilter === "true" || activeFilter === true;
      reportFormats = reportFormats.filter((item: any) => item.isActive === isActiveFilter);
    }
