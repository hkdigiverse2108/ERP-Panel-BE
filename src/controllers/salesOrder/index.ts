...
const formatSalesOrder = (so: any) => {
  const soObj = so.toObject ? so.toObject() : so;
  if (soObj.items) {
    soObj.items = soObj.items.map((item: any) => {
...
export const getOneSalesOrder = async (req, res) => {
  ...
  try {
    ...
    let soObj = formatSalesOrder(response);
