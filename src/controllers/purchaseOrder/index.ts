...
export const getAllPurchaseOrder = async (req, res) => {
  ...
  try {
    ...
    let response = await getDataWithSorting(purchaseOrderModel, criteria, {}, options);
    ...
    response = response.map((po: any) => {
      ...
      if (poObj.supplierId && poObj.supplierId.address) {
        ...
        if (poObj.billingAddress) {
          ...
          const billingAddr = poObj.supplierId.address.find((addr: any) => addr._id && addr._id.toString() === billingStr);
          if (billingAddr) {
