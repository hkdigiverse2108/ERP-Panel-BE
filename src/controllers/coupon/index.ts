...
export const getOneCoupon = async (req, res) => {
  ...
  try {
    ...
    const response = await getFirstMatch(couponModel, { _id: value?.id, isDeleted: false }, {}, { populate: [{ path: "companyId", select: "name" }, { path: "branchId", select: "name" }, { path: "createdBy", select: "name userType" }, { path: "customerIds.id", select: "firstName lastName" }] });
