...
export const editPaymentTerm = async (req, res) => {
    ...
    try {
        ...
        let isExist = await getFirstMatch(paymentTermsModel, { _id: value.paymentTermId, isDeleted: false }, {}, {});
...
export const deletePaymentTermById = async (req, res) => {
    ...
    try {
        ...
        if (!(await checkIdExist(paymentTermsModel, value?.id, "Payment Term", res))) return;
...
export const getAllPaymentTerm = async (req, res) => {
    ...
    try {
        ...
        const totalData = await countData(paymentTermsModel, criteria);
