...
import {
  addSalesCreditNoteSchema,
  deleteSalesCreditNoteSchema,
  editSalesCreditNoteSchema,
  getSalesCreditNoteSchema,
} from "../../validation";
...
export const addSalesCreditNote = async (req, res) => {
  ...
  try {
    ...
    if (value.billingAddress) {
      const isBillingValid = customer?.address?.find(
        (addr: any) =>
    ...
    if (value.shippingAddress) {
      const isShippingValid = customer?.address?.find(
        (addr: any) =>
    ...
    if (
      value?.salesId &&
      !(await checkIdExist(SalesOrderModel, value?.salesId, "Sales Order", res))
    )
      return;
    ...
    if (
      value?.salesManId &&
      !(await checkIdExist(employeeModel, value?.salesManId, "Salesman", res))
    )
      return;
    ...
    return res
      ...
      .json(
        new apiResponse(
          HTTP_STATUS.OK,
          responseMessage?.addDataSuccess("Sales Credit Note"),
          response,
          {},
        ),
      );
...
export const editSalesCreditNote = async (req, res) => {
  ...
  try {
    ...
    const isExist = await getFirstMatch(
      salesCreditNoteModel,
      { _id: value?.salesCreditNoteId, isDeleted: false },
      {},
      {},
    );
      ...
      return res
        ...
        .json(
          new apiResponse(
            ...
            responseMessage?.getDataNotFound("Sales Credit Note"),
            {},
            {},
          ),
        );
    ...
    if (
      ...
      customerForAddress = await getFirstMatch(
        contactModel,
        { _id: value.customerId, isDeleted: false },
        {},
        {},
      ...
      customerForAddress = await getFirstMatch(
        contactModel,
        { _id: isExist.customerId, isDeleted: false },
        {},
        {},
      );
      if (!customerForAddress) {
    ...
    if (customerForAddress) {
      if (value.billingAddress) {
        const isBillingValid = customerForAddress?.address?.find(
          (addr: any) =>
            addr._id && addr._id.toString() === value.billingAddress.toString(),
        );
        if (!isBillingValid) {
      ...
      if (value.shippingAddress) {
        const isShippingValid = customerForAddress?.address?.find(
          (addr: any) =>
            addr._id &&
    ...
    if (
      value?.salesManId &&
      value?.salesManId !== isExist?.salesManId?.toString()
    ) {
      if (
        !(await checkIdExist(employeeModel, value?.salesManId, "Salesman", res))
      )
    ...
    const response = await updateData(
      salesCreditNoteModel,
      { _id: new ObjectId(value?.salesCreditNoteId) },
      value,
      {},
    );
    ...
    if (!response) {
      return res
        ...
        .json(
          new apiResponse(
            HTTP_STATUS.NOT_IMPLEMENTED,
            responseMessage?.updateDataError("Sales Credit Note"),
            {},
            {},
          ),
        );
    ...
    return res
      ...
      .json(
        new apiResponse(
          HTTP_STATUS.OK,
          responseMessage?.updateDataSuccess("Sales Credit Note"),
          response,
          {},
        ),
      );
  ...
  try {
    ...
    if (!response) {
      return res
        ...
        .json(
          new apiResponse(
            HTTP_STATUS.NOT_IMPLEMENTED,
            responseMessage?.deleteDataError("Sales Credit Note"),
            {},
            {},
          ),
        );
    ...
    return res
      ...
      .json(
        new apiResponse(
          HTTP_STATUS.OK,
          responseMessage?.deleteDataSuccess("Sales Credit Note"),
          response,
          {},
        ),
      );
  ...
  try {
    ...
    response = response.map((scn: any) => {
      ...
      if (scnObj.customerId && scnObj.customerId.address) {
        ...
        scnObj.customerId.address =
          scnObj.customerId.address.map(extractAddressFields);

        if (scnObj.billingAddress) {
          const billingStr = scnObj.billingAddress.toString();
          const billingAddr = scnObj.customerId.address.find(
        ...
        if (scnObj.shippingAddress) {
          ...
          const shippingAddr = scnObj.customerId.address.find(
    ...
    return res
      ...
      .json(
        new apiResponse(
          HTTP_STATUS.OK,
          responseMessage?.getDataSuccess("Sales Credit Note"),
          { salesCreditNote_data: response, totalData, state },
          {},
        ),
      );
  } catch (error) {
  ...
  try {
    ...
    return res
      ...
      .json(
        new apiResponse(
          HTTP_STATUS.OK,
          responseMessage?.getDataSuccess("Sales Credit Note Dropdown"),
          dropdownData,
          {},
        ),
      );
