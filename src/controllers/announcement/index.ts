...
import { countData, createOne, getDataWithSorting, getFirstMatch, redisGet, redisSet, redisdelPattern, reqInfo, responseMessage, updateData, applyDateFilter } from "../../helper";
...
export const addAnnouncement = async (req, res) => {
  ...
  try {
    ...
    await redisdelPattern("announcement:*");
...
export const getAllAnnouncement = async (req, res) => {
  reqInfo(req);
  try {
    const cacheKey = `announcement:all:req:${JSON.stringify(req.query)}`;
    const cachedData = await redisGet(cacheKey);
    if (cachedData) {
      return res.status(HTTP_STATUS.OK).json(new apiResponse(HTTP_STATUS.OK, responseMessage?.getDataSuccess("Announcement"), cachedData, {}));
    }
    ...
    const responsePayload = { announcement_data: response, totalData, state: stateObj };
    await redisSet(cacheKey, responsePayload, 3600);

    return res.status(HTTP_STATUS.OK).json(new apiResponse(HTTP_STATUS.OK, responseMessage?.getDataSuccess("Announcement"), responsePayload, {}));
  } catch (error) {
    console.error(error);
    return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).json(new apiResponse(HTTP_STATUS.INTERNAL_SERVER_ERROR, responseMessage?.internalServerError, {}, error));
  }
...
export const getAnnouncementById = async (req, res) => {
  reqInfo(req);
  try {
    const cacheKey = `announcement:one:req:${JSON.stringify(req.params)}`;
    const cachedData = await redisGet(cacheKey);
    if (cachedData) {
      return res.status(HTTP_STATUS.OK).json(new apiResponse(HTTP_STATUS.OK, responseMessage?.getDataSuccess("Announcement details"), cachedData, {}));
    }
