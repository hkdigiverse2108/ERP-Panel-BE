...
export const addReturnPosOrderSchema = Joi.object({
    ...
    total: Joi.number().min(0).required(),
    type: Joi.string().valid(...Object.values(RETURN_POS_ORDER_TYPE)).default(RETURN_POS_ORDER_TYPE.SALES_RETURN),
...
export const editReturnPosOrderSchema = Joi.object({
    ...
    type: Joi.string().valid(...Object.values(RETURN_POS_ORDER_TYPE)).optional(),
    reason: Joi.string().optional().allow("", null),
