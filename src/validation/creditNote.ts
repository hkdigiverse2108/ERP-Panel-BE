...
const creditNoteItemSchema = Joi.object().keys({
...
export const addCreditNoteSchema = Joi.object().keys({
  ...
  items: Joi.array().items(creditNoteItemSchema).min(1).required(),
...
export const editCreditNoteSchema = Joi.object().keys({
  ...
  customerName: Joi.string().optional(),
  invoiceId: objectId().optional().allow("", null),
  items: Joi.array().items(creditNoteItemSchema).optional(),
